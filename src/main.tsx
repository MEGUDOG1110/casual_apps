    ...
    <div className="min-h-screen w-full flex items-start justify-center bg-[var(--md3-bg)] p-8">
      ...
      >
        ...
        <div className="flex items-center gap-3 px-7 pt-8 pb-4">
          <span
            className="material-symbols-outlined flex items-center justify-center rounded-2xl text-2xl"
            style={{ background: "#107C41", color: "#fff", width: 40, height: 40, display: "flex" }}
          >
            apps
          </span>
          <span className="text-base font-bold tracking-wide" style={{ color: "#80e27e" }}>
            アプリ一覧
          </span>
        </div>
          ...
          <ul className="space-y-1">
            {apps.map((app) => (
              <li key={app.id}>
                ...
                >
                  <span className="material-symbols-outlined text-lg" style={{ color: app.color }}>
                    {app.icon}
                  </span>
                  <span className="text-[var(--md3-on-surface)]" style={{ color: "inherit" }}>
                    {app.title}
                  </span>
                </a>
              </li>
      ...
      <main className="w-[640px] max-w-full py-0">
        ...
        <header className="rounded-[28px] bg-[var(--md3-surface)] px-10 pb-8 pt-8 shadow-[var(--md3-shadow)] mb-8 border border-[var(--md3-card-outline)]">
          <h1 className="text-2xl font-bold tracking-tight text-[var(--md3-on-surface)] mb-2">
            業務ツール ランチャー
          </h1>
          <p className="text-sm text-[var(--md3-on-surface-variant)]">
            よく使う業務アプリをまとめて管理・起動できます。
          </p>
        </header>
            ...
            return (
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  <h2 className="truncate text-lg font-semibold text-[var(--md3-on-surface)]">
                    {app.title}
                  </h2>
                  <p className="mt-1 text-sm text-[var(--md3-on-surface-variant)] truncate">
                    {app.description}
                  </p>
                ...
                <div className="flex flex-col items-end gap-2">
                  ...
                  <span className="rounded-full border border-[var(--md3-badge-outline)] px-4 py-1 text-xs text-[var(--md3-on-surface-variant)] bg-[var(--md3-badge-bg)]">
                    dist/app/{app.id}
                  </span>
                </div>
              </article>
